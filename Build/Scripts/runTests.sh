...
#
# TYPO3 extension cute-mailing test runner based on docker.
#
...
USERSET=""
SUFFIX="$RANDOM"
NETWORK="typo3-cute-mailing-${SUFFIX}"
CI_PARAMS="${CI_PARAMS:-}"
...
case ${TEST_SUITE} in
    ...
    fixComposerNormalize)
        ...
        ;;
    functional)
        COMMAND=(.Build/bin/phpunit -c Tests/FunctionalTests.xml --exclude-group not-${DBMS} "$@")
        case ${DBMS} in
            mariadb)
    ...
    shellcheck)
        ...
        ;;
    unit)
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name unit-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${IMAGE_PHP} .Build/bin/phpunit -c Tests/UnitTests.xml "$@"
        SUITE_EXIT_CODE=$?
        ;;
    unitRandom)
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name unit-random-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${IMAGE_PHP} .Build/bin/phpunit -c Tests/UnitTests.xml --order-by=random ${PHPUNIT_RANDOM} "$@"
        SUITE_EXIT_CODE=$?
        ;;
    update)
